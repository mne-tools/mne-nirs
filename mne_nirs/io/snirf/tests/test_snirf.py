...
def test_snirf_write_raw(fname, tmpdir):
    ...
    assert_allclose(
        [float(d) for d in raw.annotations.description],
    ...
    assert_array_equal(raw.info.get_channel_types(), raw_orig.info.get_channel_types())

    assert abs(raw_orig.info["meas_date"] - raw.info["meas_date"]) < datetime.timedelta(
        seconds=1
    )
    ...
    for line in obj_diff.splitlines():
        if (
            ("logno" not in line)
            and ("scanno" not in line)
            and ("his_id" not in line)
            and ("dig" not in line)
            and ("datetime mismatch" not in line)
        ):
...
def _verify_snirf_required_fields(test_file):
    ...
    with h5py.File(test_file, "r") as h5:
        ...
        measurement_lists = [k for k in data1.keys() if k.startswith("measurementList")]
...
def test_snirf_stim_roundtrip(fname, tmpdir):
    ...
    raw = read_raw_snirf(test_file)
    assert_array_equal(raw_orig.annotations.onset, raw.annotations.onset)
    assert_array_equal(raw_orig.annotations.duration, raw.annotations.duration)
    assert_array_equal(raw_orig.annotations.description, raw.annotations.description)
...
@pytest.mark.parametrize("newduration", (1, 2, 3))
...
def test_optical_density_roundtrip(fname, tmpdir):
    ...
    od = read_raw_snirf(test_file)
    assert "fnirs_od" in od
    assert_array_equal(od_orig.annotations.onset, od.annotations.onset)
    assert_array_equal(od_orig.annotations.duration, od.annotations.duration)
    assert_array_equal(od_orig.annotations.description, od.annotations.description)
    assert_array_equal(od_orig.get_data(), od.get_data())
    assert_array_equal(od_orig.info.get_channel_types(), od.info.get_channel_types())
...
def test_haemoglobin_roundtrip(fname, tmpdir):
    ...
    assert "hbo" in hb
    assert "hbr" in hb
    assert_array_equal(hb_orig.annotations.onset, hb.annotations.onset)
    assert_array_equal(hb_orig.annotations.duration, hb.annotations.duration)
    assert_array_equal(hb_orig.annotations.description, hb.annotations.description)
    assert_array_equal(hb_orig.get_data(), hb.get_data())
    assert_array_equal(hb_orig.info.get_channel_types(), hb.info.get_channel_types())
    #
    # # HBO
    #
