...
def test_results_glm_export_dataframe_region_of_interest():
    ...
    df = res.to_dataframe_region_of_interest(
        rois, ["e1p0", "e3p0", "drift_1"], demographic_info=True
    )
...
def test_results_glm_export_dataframe_region_of_interest_weighted():
    ...
    assert np.isclose(df_uw.query("ROI == 'A'").theta, thetas[rois["A"]].mean())
    ...
    with pytest.raises(ValueError, match="must be positive"):
        weights["C"] = [16, 7, -8, 9]
        _ = res.to_dataframe_region_of_interest(rois, "e1p0", weighted=weights)
    ...
    with pytest.raises(KeyError, match="Keys of group_by and weighted"):
        bad_weights = dict()
        bad_weights["Z"] = [0, 2, 4]
        _ = res.to_dataframe_region_of_interest(rois, "e1p0", weighted=bad_weights)
