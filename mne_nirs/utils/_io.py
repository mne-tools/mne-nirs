...
def glm_to_tidy(info, statistic, design_matrix, wide=True, order=None):
    ...
    if isinstance(statistic, dict) and isinstance(
        statistic[list(statistic.keys())[0]], nilearn.glm.regression.RegressionResults
    ):
...
def _tidy_RegressionResults(data, glm_est, design_matrix):
    ...
    for idx, name in enumerate(glm_est.keys()):
        ...
        for cond_idx, cond in enumerate(design_matrix.columns):
            t_estimates[idx, cond_idx] = glm_est[name].t(column=cond_idx).item()
            p_estimates[idx, cond_idx] = 2 * stats.t.cdf(
                -1.0 * np.abs(t_estimates[idx, cond_idx]),
                df=df_estimates[idx, cond_idx],
            )
            se_estimates[idx, cond_idx] = np.sqrt(np.diag(glm_est[name].vcov()))[
                cond_idx
            ]

    list_vals = [0] * (len(data.ch_names) * len(design_matrix.columns) * 6)
    idx = 0
...
def _tidy_long_to_wide(d, expand_output=True):
    ...
    if expand_output:
        try:
            ...
            d["Chroma"] = [
                re.search(r"S(\d+)_D(\d+) (\w+)", ch).group(3) for ch in d["ch_name"]
            ]
