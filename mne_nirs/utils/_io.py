...
def _GLM_to_tidy_long(data, labels, glm_estimates, design_matrix):
    ...
    for idx, lab in enumerate(labels):
        ...
        for cond_idx, cond in enumerate(design_matrix.columns):
            t_estimates[idx, cond_idx] = glm_estimates[lab].t(column=cond_idx)[matching_idx]
    ...
    for ch_idx, ch in enumerate(data.ch_names):
        for cond_idx, cond in enumerate(design_matrix.columns):
            df = df.append(
                {'ch_name': ch, 'condition': cond, 'variable': "theta",
                 'value': theta_estimates[ch_idx][cond_idx]}, ignore_index=True)
            df = df.append({'ch_name': ch, 'condition': cond, 'variable': "t",
                            'value': t_estimates[ch_idx][cond_idx]},
                           ignore_index=True)
            df = df.append({'ch_name': ch, 'condition': cond, 'variable': "df",
                            'value': df_estimates[ch_idx][cond_idx]},
                           ignore_index=True)
            df = df.append({'ch_name': ch, 'condition': cond, 'variable': "p",
                            'value': p_estimates[ch_idx][cond_idx]},
                           ignore_index=True)
            df = df.append({'ch_name': ch, 'condition': cond, 'variable': "mse",
...
def _tidy_long_to_wide(d):
    ...
    d = d.pivot_table( columns='variable', values='value', index=['ch_name', 'condition'])
    d["Sig"] = d["p"] < 0.05
