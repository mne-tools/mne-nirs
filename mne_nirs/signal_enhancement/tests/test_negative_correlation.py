...
def test_cui():
    ...
    with pytest.raises(RuntimeError, match="run on haemoglobin"):
        _ = mne_nirs.signal_enhancement.enhance_negative_correlation(raw_intensity)
    raw_od = mne.preprocessing.nirs.optical_density(raw_intensity)
    with pytest.raises(RuntimeError, match="run on haemoglobin"):
        _ = mne_nirs.signal_enhancement.enhance_negative_correlation(raw_od)
    raw_haemo = mne.preprocessing.nirs.beer_lambert_law(raw_od, ppf=0.1)
    raw_anti = mne_nirs.signal_enhancement.enhance_negative_correlation(raw_haemo)
    assert np.abs(np.corrcoef(raw_haemo._data[0], raw_haemo._data[1])[0, 1]) < 1

    np.testing.assert_almost_equal(
        np.corrcoef(raw_anti._data[0], raw_anti._data[1])[0, 1], -1
    )
