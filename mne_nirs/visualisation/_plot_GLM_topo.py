...
def plot_glm_group_topo(
    ...
    *,
    ...
    if show_names is not None:
        names = show_names
        warn(
            "show_names is deprecated and will be removed in the next "
            "release, use names instead",
            FutureWarning,
        )
    del show_names
    ...
    if not (info.ch_names == list(statsmodel_df["ch_name"].values)):
        if len(info.ch_names) < len(list(statsmodel_df["ch_name"].values)):
            ...
            statsmodel_df["Keep"] = [
                g in info.ch_names for g in statsmodel_df["ch_name"]
            ]
...
def _handle_vlim(vlim, vmin, vmax, estimates):
    if vmin is not None or vmax is not None:
        warn(
            "vmin and vmax are deprecated and will be removed in the next "
