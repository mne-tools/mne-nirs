...
if [ ! -z "$CONDA_ENV" ]; then
	...
	pip install --progress-bar off --upgrade --pre --only-binary ":all:" --no-deps -i "https://pypi.anaconda.org/scipy-wheels-nightly/simple" numpy  "scipy<=1.7.2" pandas "scikit-learn>=0.24.2" dipy
