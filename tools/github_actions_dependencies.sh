...
if [ ! -z "$CONDA_ENV" ]; then
	...
	pip install --progress-bar off --upgrade --pre --only-binary ":all:" scipy
	pip install --progress-bar off --upgrade --pre --only-binary ":all:" -i "https://pypi.anaconda.org/scipy-wheels-nightly/simple" numpy  pandas scikit-learn
...
pip install --progress-bar off --upgrade -r https://github.com/mne-tools/mne-python/archive/main.zip
